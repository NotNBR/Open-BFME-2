...
extern "C" __declspec(dllimport) void *__stdcall CreateFileMappingA(
	void *, void *, unsigned long, unsigned long, unsigned long, char const *);
extern "C" __declspec(dllimport) void *__stdcall MapViewOfFile(
	void *, unsigned long, unsigned long, unsigned long, unsigned int);
...
void *_Filebuf_base::_M_mmap(streamoff offset, streamoff len)
{
	...
	LARGE_INTEGER mapOff;
	mapOff.QuadPart = offset;
	base = MapViewOfFile(_M_view_id, FILE_MAP_READ,
		mapOff.HighPart, mapOff.LowPart, (unsigned int)len);
	if (base == 0) {
		goto close_view;
	}
	if (offset + len < 0) {
	...
	li.LowPart = SetFilePointer(_M_file_id, li.LowPart, &li.HighPart, FILE_BEGIN);
	if (li.LowPart == (unsigned long)-1 && GetLastError() != 0) {
		goto fail_unmap;
	}
	if ((long)li.LowPart >= 0) {
		return base;
	}
fail_unmap:
