...
// Sacrificial DIR32 anchor: retail pushes the codec lookup table at
// VA 0x00BBE164 here (BFME1 donor baked its own 0x0107C7B4 VA as an
// immediate). The address slot patches from retail; content unchecked.
static unsigned char codecGridLookupTable[256];
			...
			if (width > 0)
			{
				...
				do
				{
					...
					((Rva009B4390Four)Rva009B4390CodecCall)(
						self, offset, (void *)codecGridLookupTable, value);
					++column;
					columnOffset += 0x10;
				} while (column < width);
			}
