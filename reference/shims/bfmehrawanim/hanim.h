// Header shim: BFME2's HAnimClass declares the quaternion orientation query
// with a bool report where Zero Hour's is void. Retail HRawAnimClass::
// Get_Orientation at 0x0018DD60 sets al=1 on every return path, and BFME1's
// hanim.h carries the same bool revision (HTreeClass::Anim_Update tests the
// result and skips the rotation post-multiply when false). Only that one
// declaration changes here; the vtable order is untouched, so every other
// slot keeps its Zero Hour index for this translation unit.
...
//	virtual Quaternion		Get_Orientation(int pividx,float frame) = 0;
	...
	// BFME reports whether the pivot has rotation (see shim note above).
